class JoinButton extends HTMLElement {
  constructor() {
    super();
  }
  ...
  connectedCallback() {
    this.innerHTML = `
      ...
      <button class="button" id="joinBtn">
        <div class="bg"></div>
        <svg
    ...
    // Event-Listener für den Join-Button
    this.querySelector('#joinBtn').addEventListener('click', () => {
      this.dispatchEvent(new CustomEvent('join-clicked'));
    });
  }
}

customElements.define('join-button', JoinButton);